...
export interface Item<IdType extends string | number = string> {
  id?: IdType
  text: string
  imageHref?: string
  disabled?: boolean
...
export const exampleItems: Item<string>[] = [
  {
    id: "1",
    text: "Wade Cooper",
    imageHref:
...
interface TypeaheadDropdownProps<ItemIdType extends number | string> {
  items: Item<ItemIdType>[]
  label: Label
  onChange: ({
    selected,
    userGenerated,
  }: {
    selected: Record<ItemIdType, Item<ItemIdType>>
  ...
  defaultSelection?: Record<ItemIdType, Item<ItemIdType>>
  disableDeselect?: boolean
  allowFiltering?: boolean
}

export const exampleProps: TypeaheadDropdownProps<string> = {
  items: exampleItems,
  label: { text: "Assigned To" },
  onChange: () => {},
}

export const TypeaheadDropdown = <ItemIdType extends number | string>({
  items,
  label,
  onChange,
  multiSelect = false,
  ...
  disableDeselect,
  allowFiltering = true,
}: TypeaheadDropdownProps<ItemIdType>) => {
  const [selected, setSelected] = useState(
    defaultSelection || ({} as Record<ItemIdType, Item<ItemIdType>>)
  )
  ...
  useDeepCompareEffect(() => {
    ...
    onChange({
      selected: selectedWithoutUserGenerated as Record<
        ItemIdType,
        Item<ItemIdType>
      >,
      userGenerated,
  ...
  const addOrRemoveSelection = (newSelection: Item<ItemIdType>) => {
    if (newSelection.disabled) return

    const selectedClone = multiSelect
      ? { ...selected }
      : ({} as typeof selected)
  ...
  const filterItems = () => {
    ...
    if (searchItem && allowUserGeneratedOptions) {
      const otherItem: Item<ItemIdType> = {
        text: `${userGeneratedOptionLabel}: ${searchItem}`,
        id: `${userGeneratedOptionLabel}::${Date.now()}` as ItemIdType,
      }
  ...
  const selectedItems = Object.values(selected) as Item<ItemIdType>[]
  ...
  const classesForUnselectedElement =
    "relative py-1 pl-1 text-gray-900 mb-1 cursor-default select-none pr-9"

  const selectAllClasses =
    "text-primary-400 text-sm cursor-pointer flex-shrink-0"

  const selectAll = () => {
    const allSelected = {} as Record<ItemIdType, Item<ItemIdType>>
    items.forEach((item) => (allSelected[item.id] = { ...item }))
    setSelected(() => ({ ...selected, ...allSelected }))
  }

  const clearAll = () => {
    setSelected({} as Record<ItemIdType, Item<ItemIdType>>)
  }
          ...
          {truncatedSelection.map((item) => (
            ...
            >
              {item.imageHref && (
                <div
                  className={`mr-2 ${
                    item.imgSize ? item.imgSize : "h-6 w-6"
